...
//树枝的阈值，java8之前解决hash冲突是通过链表的方式，java8中引入了红黑树，当某个hash节点冲突大于`TREEIFY_THRESHOLD`，使用红黑树，从而大大的提高查找效率
    ...
    final Node<K,V>[] resize() {
        ...
        int newCap, newThr = 0;
        if (oldCap > 0) {
            //老table长度是不会大于MAXIMUM_CAPACITY，最多等于，此处只是为了防止特殊情况，写的大于等于。这种情况下不进行扩容。
            if (oldCap >= MAXIMUM_CAPACITY) {
                threshold = Integer.MAX_VALUE;
                return oldTab;
            }
            //如果newCap<最大容量并且oldCap>=16, 则将新阈值设置为原来的两倍
        ...
        // 2.如果老表的容量为0, 老表的阈值大于0, 是因为初始容量被放入阈值，则将新表的容量设置为老表的阈值
            ...
            newCap = oldThr;
        // 3.老表的容量为0, 老表的阈值为0，这种情况是没有传初始容量的new方法创建的空表，将阈值和容量设置为默认值（默认容量为16，默认加载因子为0.75，则默认阈值为16*0.75=12）
